...
from .semantics import site_rmse
...
class FR5LowLevelGraspEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(
        ...
        self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(89,), dtype=np.float32)
    ...
    def _solve_actuated_targets(
        ...
        arm_qpos = solve_arm_wrist_palm_ik(
            ...
            target_sites_world=wrist_pose_to_target_sites(wrist_pose_world),
    ...
    def _object_relative_pose_features(self, semantic_sites_world: np.ndarray, object_pose: np.ndarray) -> np.ndarray:
        palm_frame = quat_wxyz_to_matrix(wrist_pose_from_semantic_sites(semantic_sites_world)[3:])
        palm_center = semantic_sites_world[1]
        object_rotation = quat_wxyz_to_matrix(object_pose[3:])
        rel_position = palm_frame.T @ (object_pose[:3] - palm_center)
        rel_rotation = palm_frame.T @ object_rotation
        return np.concatenate([rel_position, rotation_6d_from_matrix(rel_rotation)])

    def _pose_goal_features(self) -> np.ndarray:
        ...
        return np.concatenate([target_wrist_pose_object[:3], rotation_6d_from_matrix(target_rotation)])
    ...
    def _observation(self) -> np.ndarray:
        ...
        object_velocity = np.concatenate(
            [palm_frame.T @ object_linear_vel, palm_frame.T @ object_angular_vel]
        )
        obs = np.concatenate(
            [
                self.runtime.get_actuated_qpos(),
                self.runtime.get_actuated_qvel(),
                self._object_relative_pose_features(semantic_sites_world, object_pose),
                object_velocity,
                self._pose_goal_features(),
                np.asarray(self.current_goal.hand_qpos_6, dtype=np.float64),
                np.asarray(self.current_goal.semantic_sites_goal_object_21, dtype=np.float64),
                np.asarray(self.current_goal.contact_mask_12, dtype=np.float64),
                self._phase_one_hot(),
            ]
        )
    ...
    def _compute_reward(self, action: np.ndarray) -> tuple[float, dict[str, float]]:
        ...
        current_wrist_pose_object = compose_pose7(
            ...
            wrist_pose_from_semantic_sites(current_sites_world),
        )
        target_wrist_pose_object = self._current_goal_wrist_pose_object()
        ...
        actual_contact_mask = np.asarray(contact_diag["mask"], dtype=np.float64)
        ...
        object_linear_vel, object_angular_vel = self.runtime.get_object_velocity()
        palm_linear_vel, palm_angular_vel = self.runtime.get_site_velocity("palm_center")
        ...
        wrist_translation_error = float(
            np.linalg.norm(current_wrist_pose_object[:3] - target_wrist_pose_object[:3])
        )
        wrist_rotation_error_deg = float(rotation_angle_deg(current_wrist_frame, target_wrist_frame))
        semantic_site_rmse = float(site_rmse(current_sites_obj, target_sites_obj, include_wrist=True))
        ...
        contact_term = 0.7 * matched_positive + 0.3 * contact_bit_match - 0.25 * false_positive
        ...
        body_vel_term = float(np.dot(palm_linear_vel, palm_linear_vel))
        body_qvel_term = float(np.dot(palm_angular_vel, palm_angular_vel))
    ...
    def step(self, action: np.ndarray):
        ...
        self.current_wrist_target_pose = apply_local_pose_delta(
            self.current_wrist_target_pose,
            wrist_translation_delta,
            wrist_rotation_delta,
        )
        self.current_hand_target_qpos = self.runtime.clamp_hand(self.current_hand_target_qpos + hand_delta)
            ...
            info.update(
                {
                    ...
                    "episode_success": float(not self.slipped),
